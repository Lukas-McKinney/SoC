...
static bool test_settlement_can_use_corner_with_one_opponent_road(void);
...
int main(void)
{
    const struct
    {
        ...
        {"settlement allowed with one opponent road", test_settlement_can_use_corner_with_one_opponent_road},
    ...
    for (int nodeIndex = 0; nodeIndex < topology.nodeCount; nodeIndex++)
    {
...

        {
            ...
            ASSERT_FALSE(boardIsValidSettlementPlacement(&map, node->tileId, node->cornerIndex,
                                                         PLAYER_RED, kBoardOrigin, kBoardRadius));
            ASSERT_TRUE(strcmp(boardGetSettlementPlacementFailureReason(&map, node->tileId, node->cornerIndex,
                                                                        PLAYER_RED, kBoardOrigin, kBoardRadius),
                               "corner blocks another player's road") == 0);
        }
...
static bool test_settlement_can_use_corner_with_one_opponent_road(void)
{
    ...
    for (int nodeIndex = 0; nodeIndex < topology.nodeCount; nodeIndex++)
    {
        ...
        {
            const struct NodeRef *node = &topology.nodes[nodeIndex];
            ASSERT_TRUE(boardIsValidSettlementPlacement(&map, node->tileId, node->cornerIndex,
                                                        PLAYER_RED, kBoardOrigin, kBoardRadius));
